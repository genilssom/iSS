...
//***************************************************************************
FSSW::FSSW(
    std::shared_ptr<RandomUtil::Random> ran_gen, Table *chosen_particles_in,
    ...
    : path_(path),
      ...
      afterburner_type_(afterburner_type),
    ...
    if (paraRdr->getVal("include_deltaf_shear", 1) == 0) {
        INCLUDE_SHEAR_DELTAF = false;
    } else {
        INCLUDE_SHEAR_DELTAF = true;
    }
    if (paraRdr->getVal("include_deltaf_bulk", 1) == 0) {
        INCLUDE_BULK_DELTAF = false;
    } else {
        INCLUDE_BULK_DELTAF = true;
    }
    if (paraRdr->getVal("include_deltaf_diffusion", 0) == 0) {
        INCLUDE_DIFFUSION_DELTAF = false;
    } else {
        INCLUDE_DIFFUSION_DELTAF = true;
    }

    deltaf_kind_ = paraRdr->getVal("deltaf_kind", 20);
    ...
    if (paraRdr->getVal("perform_decays") == 1) {
        ...
        decayer_ptr_ = std::unique_ptr<particle_decay>(
            new particle_decay(ran_gen_ptr, afterburner_type_, table_path_));
    } else {
    ...
    if (flag_include_spectators != 0) {
        ...
        spectators_ptr_ = std::unique_ptr<Spectators>(
            new Spectators(flag_include_spectators));
        ...
        if (deltaf_kind_ == 0) {
            bulkdf_coeff_ =
                std::unique_ptr<Table>(new Table(
                    table_path_
                    + "/deltaf_tables/"
                      "BulkDf_Coefficients_Hadrons_s95p-v0-PCE.dat"));
...
inline long FSSW::determine_number_to_sample(
    ...
    // From a non-integer averaged particles number dN, return an actual
    // interger particle number that can be used in sampling.
    if (dN_dy_in < 0) {
        cout << "FSSW::"
    ...
    switch (model)  // explained in parameters.dat
    {
        ...
        default:
            cout << "FSSW::"
                 ...
                 << "to determine the number of particles is not found" << endl;
            exit(-1);
    }
...
bool FSSW::particles_are_the_same(int idx1, int idx2) {
    ...
    if (particles[idx1].baryon != particles[idx2].baryon) return false;
    if (particles[idx1].strange != particles[idx2].strange) return false;
    if (particles[idx1].charge != particles[idx2].charge) return false;
    double tolerance = paraRdr->getVal("grouping_tolerance");
    if (std::abs(
            (particles[idx1].mass - particles[idx2].mass)
            / (particles[idx2].mass + 1e-30))
        > tolerance)
        return false;
    if (flag_PCE_ == 1) {
...
void FSSW::shell() {
    ...
    if (flag_spectators_) addSpectatorsToHadronList();
...
void FSSW::combine_samples_to_OSCAR() {
    ...
    if (flag_store_samples_in_memory == 0) {
        ...
        for (int m = 0; m < number_of_chosen_particles; m++) {
            ...
            samples_control_filename << path_ << "/samples_control_" << monval
                                     << ".dat";
            ...
            if (!controls[m]->is_open()) {
                cout << endl
                     ...
                     << samples_control_filename.str() << " not found." << endl;
                exit(-1);
            }
        ...
        for (long sample_idx = 1; sample_idx <= number_of_repeated_sampling;
            ...
            oscar << setw(10) << sample_idx << "  " << setw(10)
                  << total_number_of_particles << "  " << setw(8) << 0.0 << "  "
                  << setw(8) << 0.0 << endl;
        ...
        for (unsigned int iev = 0; iev < Hadron_list->size(); iev++) {
            ...
            if (total_number_of_particles > 0) {
                // sub-header for each event
                oscar << setw(10) << iev << "  " << setw(10)
                      << total_number_of_particles << "  " << setw(8) << 0.0
                      << "  " << setw(8) << 0.0 << endl;
                for (int ipart = 0; ipart < total_number_of_particles;
                     ipart++) {
                    oscar << setw(10) << ipart + 1 << "  " << setw(10)
                          << (*(*Hadron_list)[iev])[ipart].pid << "  ";
                    snprintf(
                        line_buffer, 500,
                        "%24.16e  %24.16e  %24.16e  %24.16e  %24.16e  %24.16e  "
...
void FSSW::combine_samples_to_gzip_file() {
    ...
    for (auto const &ev_i : (*Hadron_list)) {
        ...
        for (auto &part_i : (*ev_i)) {
            ...
            gzprintf(
                fp_gz, "%.7e %.7e %.7e %.7e %.7e %.7e %.7e %.7e %.7e\n",
                part_i.mass, part_i.t, part_i.x, part_i.y, part_i.z, part_i.E,
                part_i.px, part_i.py, part_i.pz);
        }
    }
    gzclose(fp_gz);
    ...
    sw.toc();
    cout << endl
         << " -- combine_samples_to_gzip_file finishes " << sw.takeTime()
         << " seconds." << endl;
}
...
void FSSW::combine_samples_to_binary_file() {
    ...
    for (auto const &ev_i : (*Hadron_list)) {
        ...
        for (auto &part_i : (*ev_i)) {
            ...
            float array[] = {
                static_cast<float>(part_i.mass), static_cast<float>(part_i.t),
                static_cast<float>(part_i.x),    static_cast<float>(part_i.y),
                static_cast<float>(part_i.z),    static_cast<float>(part_i.E),
                static_cast<float>(part_i.px),   static_cast<float>(part_i.py),
                static_cast<float>(part_i.pz),
            };
    ...
    sw.toc();
    cout << endl
         << " -- combine_samples_to_binary_file finishes " << sw.takeTime()
         << " seconds." << endl;
}
...
void FSSW::calculate_dN_dxtdy_for_one_particle_species(
    ...
    */
    ...
    const double unit_factor =
        1.0 / pow(hbarC, 3);  // unit: convert to unitless
    ...
    for (long l = 0; l < FO_length; l++) {
        ...
        if (NEoS_deltaf_kind_ == 1) {
            getCENEOSBQSCoefficients(surf->Edec, surf->Bn, visCoefficients);
        } else if (NEoS_deltaf_kind_ == 0) {
        ...
        if (INCLUDE_DIFFUSION_DELTAF) {
            dsigma_dot_q =
                (surf->qmuLRF_x * surf->da_mu_LRF[1]
        ...
        if (INCLUDE_BULK_DELTAF) {
            if (deltaf_kind_ == 1) {
                deltaN_bulk =
                    (unit_factor * prefactor * dsigma_dot_u
                     * (-bulkPi * visCoefficients[0])
                     * (-visCoefficients[1] * results_ptr[1] + results_ptr[2]));
            } else if (deltaf_kind_ == 11) {
                deltaN_bulk =
                    (unit_factor * prefactor * dsigma_dot_u * bulkPi
                     * (results_ptr[1] * mass * mass * visCoefficients[0]
                        + results_ptr[2] * baryon * visCoefficients[1]
                        + results_ptr[3] * visCoefficients[2]));
                ...
                deltaN_bulk =
                    (unit_factor * prefactor * dsigma_dot_u * bulkPi
                     * (results_ptr[1] * mass * mass * visCoefficients[2]
                        + results_ptr[2]
                              * (baryon * visCoefficients[3]
                                 + strange * visCoefficients[4]
        ...
        if (INCLUDE_DIFFUSION_DELTAF) {
            deltaN_qmu =
                (unit_factor * prefactor * dsigma_dot_q / deltaf_qmu_coeff
                 * (-prefactor_qmu * results_ptr[4] - baryon * results_ptr[5]));
        }
    ...
    for (int n = 1; n <= truncate_order; n++) {
        ...
        if (std::isnan(N_eq)) {
            cout << "N_eq is nan"
                 ...
                 << ", exp(mu/T) = " << lambda << ", exp(n*mu/T) = " << fugacity
                 << ", n*m/T = " << arg << ", K_2 = " << K_2 << endl;
            exit(1);
        }

        if (INCLUDE_BULK_DELTAF) {
            double K_1 = get_special_function_K1(arg);
            if (deltaf_kind_ == 1 || deltaf_kind_ == 21) {
                // Chapman-Enskog
                deltaN_bulk_term1 +=
                    theta * fugacity * (mass * beta * K_1 + 3 * K_2 / n);
                deltaN_bulk_term2 += theta * fugacity * K_1;
            } else if (deltaf_kind_ == 11 || deltaf_kind_ == 20) {
                // 14-moment/22-moment
                double K_3 = get_special_function_K3(arg);
                deltaN_bulk_term1 += theta * fugacity * K_2;  // mass
                deltaN_bulk_term2 +=
                    theta * fugacity * (mass * beta * K_1 + 3 * K_2 / n);  // E
                deltaN_bulk_term3 +=
                    theta * fugacity
                    * (mass * beta * K_2 + 3 * K_3 / n);  // E^2
            }
        }
        ...
        if (INCLUDE_DIFFUSION_DELTAF) {
            ...
            double I_1_n = 0.0;
            double I_1_1 =
                exp(-arg) / arg * (2. / (arg * arg) + 2. / arg - 1. / 2.);
...
int FSSW::compute_number_of_sampling_needed(int number_of_particles_needed) {
    ...
    int nev_needed =
        static_cast<int>(number_of_particles_needed / (6. * dNdy_thermal_pion));
    if (hydro_mode == 2) {
        nev_needed *= 10;
    }
...
void FSSW::sample_using_dN_dxtdy_4all_particles_conventional() {
    ...
    */
    ...
    int flag_sample_upto_desired_particle_number =
        paraRdr->getVal("sample_upto_desired_particle_number");
    if (flag_sample_upto_desired_particle_number == 1) {
        long number_of_particles_needed =
            paraRdr->getVal("number_of_particles_needed");
        number_of_repeated_sampling =
            compute_number_of_sampling_needed(number_of_particles_needed);
    }
    ...
    for (int n = 0; n < number_of_chosen_particles; n++) {
        ...
        if (local_charge_conservation == 1) {
            if (particle->charge < 0) {
                cout << "local charge conservation is turn on~ "
                     << "Skip the negative charge particles." << endl;
                continue;
            }
        }
        ...
        RandomVariable1DArray rand1D(
            &dN_dxtdy_for_one_particle_species, ran_gen_ptr, 0);
        ...
        for (long repeated_sampling_idx = 1;
            ...
            long number_to_sample =
                determine_number_to_sample(dN, sampling_model, sampling_para1);
            ...
            while (sample_idx <= number_to_sample) {
                    ...
                    if (NEoS_deltaf_kind_ == -1 && deltaf_kind_ != 31) {
                        if (deltaf_kind_ == 11) {
                            ...
                            getbulkvisCoefficients(
                                surf->Tdec, surf->muB, visCoefficients);
                        } else {
                ...
                if (INCLUDE_DIFFUSION_DELTAF) {
                    deltaf_qmu_coeff =
                        get_deltaf_qmu_coeff(surf->Tdec, surf->muB);
                }

                // next sample pt and phi
                double pT, phi, y_minus_eta_s;
                int status = sample_momemtum_from_a_fluid_cell(
                    mass, sign, baryon, strange, charge, surf, visCoefficients,
                    deltaf_qmu_coeff, pT, phi, y_minus_eta_s);
                ...
                if (hydro_mode != 2) {
                    double rap =
                        (y_LB + (y_RB - y_LB) * ran_gen_ptr->rand_uniform());
                    eta_s = rap - y_minus_eta_s;
                }

                add_one_sampled_particle(
                    repeated_sampling_idx, surf, particle->monval, mass, pT,
                    phi, y_minus_eta_s, eta_s);
                ...
                if (local_charge_conservation == 1 && particle->charge > 0) {
                    ...
                    do {
                        status2 = sample_momemtum_from_a_fluid_cell(
                            mass, sign, -baryon, -strange, -charge, surf,
                            visCoefficients, deltaf_qmu_coeff, pT2, phi2,
                            y_minus_eta_s2);
                    } while (status2 == 0);
                    add_one_sampled_particle(
                        repeated_sampling_idx, surf, -particle->monval, mass,
        ...
        if (AMOUNT_OF_OUTPUT > 2) {
            cout << endl
                 << "Sampling finished in " << sw.takeTime() << " seconds."
                 << endl;
        }
...
void FSSW::getbulkvisCoefficients(
    ...
    if (deltaf_kind_ == 0) {
    ...
    } else if (deltaf_kind_ == 1) {  // relaxation type
        ...
        bulkvisCoefficients[0] =
            (642096.624265727 - 8163329.49562861 * Tdec_fm_power[1]
        ...
        bulkvisCoefficients[1] =
            (1.18171174036192 - 17.6740645873717 * Tdec_fm_power[1]
...
void FSSW::load_CE_deltaf_NEOSBQS_table(string filepath) {
    ...
    deltaf_coeff_NEOSBQS_table_de_ =
        (deltaf_coeff_CE_NEOSBQS_tb_[200][0]
...
void FSSW::getbulkvisCoefficients(
    const double Tdec, const double mu_B,
    std::vector<double> &bulkvisCoefficients) {
    bulkvisCoefficients.resize(3, 0.);
    int idx_T = static_cast<int>(
        (Tdec - deltaf_bulk_coeff_14mom_table_T0_)
        / deltaf_bulk_coeff_14mom_table_dT_);
    int idx_mu = static_cast<int>(
        (mu_B - deltaf_bulk_coeff_14mom_table_mu0_)
        / deltaf_bulk_coeff_14mom_table_dmu_);
    double x_fraction =
        ((Tdec - deltaf_bulk_coeff_14mom_table_T0_)
             / deltaf_bulk_coeff_14mom_table_dT_
...
void FSSW::getCENEOSBQSCoefficients(
    const double Edec, const double nB, std::vector<double> &visCoefficients) {
    visCoefficients.resize(3, 0.);
    int idx_e = static_cast<int>(
        (Edec - deltaf_coeff_CE_NEOSBQS_tb_[0][0])
        / deltaf_coeff_NEOSBQS_table_de_);
    // avoid overflow and underflow
    idx_e =
        std::max(0, std::min(deltaf_coeff_NEOSBQS_table_length_e_ - 2, idx_e));
    int Ne1 = idx_e * deltaf_coeff_NEOSBQS_table_length_e_;
    int Ne2 = (idx_e + 1) * deltaf_coeff_NEOSBQS_table_length_e_;
    double e_frac =
        ((Edec - deltaf_coeff_CE_NEOSBQS_tb_[Ne1][0])
         / deltaf_coeff_NEOSBQS_table_de_);
...
void FSSW::get22momNEOSBQSCoefficients(
    const double Edec, const double nB, std::vector<double> &visCoefficients) {
    visCoefficients.resize(6, 0.);
    int idx_e = static_cast<int>(
        (Edec - deltaf_coeff_22mom_NEOSBQS_tb_[0][0])
        / deltaf_coeff_NEOSBQS_table_de_);
    // avoid overflow and underflow
    idx_e =
        std::max(0, std::min(deltaf_coeff_NEOSBQS_table_length_e_ - 2, idx_e));
    ...
    int Ne2 = (idx_e + 1) * deltaf_coeff_NEOSBQS_table_length_e_;
    double e_frac =
        ((Edec - deltaf_coeff_22mom_NEOSBQS_tb_[Ne1][0])
         / deltaf_coeff_NEOSBQS_table_de_);
...
double FSSW::get_special_function_K1(double arg) {
    ...
    if (arg < sf_x_min || arg > sf_x_max - sf_dx) {
        ...
        double fraction = (arg - sf_x_min - idx * sf_dx) / sf_dx;
        results =
            ((1. - fraction) * sf_bessel_Kn[idx][0]
             + fraction * sf_bessel_Kn[idx + 1][0]);
    }
    return (results);
}

void FSSW::get_special_function_En(double arg, std::vector<double> &results) {
    if (arg < sf_x_min || arg > sf_x_max - sf_dx) {
        if (AMOUNT_OF_OUTPUT > 5) {
...
void FSSW::check_samples_in_memory() {
    ...
    for (unsigned int i = 0; i < Hadron_list->size(); i++) {
        cout << "event " << i
             << ": number of particles = " << (*Hadron_list)[i]->size() << endl;
        for (unsigned int j = 0; j < (*Hadron_list)[i]->size(); j++) {
            cout << "particle " << j << " pid = " << (*(*Hadron_list)[i])[j].pid
                 << " mass = " << (*(*Hadron_list)[i])[j].mass << endl;
        }
    }
}
...
double FSSW::get_deltaf_bulk(
    ...
    const double Tdec, const int sign, const int baryon, const int strange,
    const int charge, const double f0,
    const std::vector<double> bulkvisCoefficients) {
    if (!INCLUDE_BULK_DELTAF) return (0.0);
    ...
    if (deltaf_kind_ == 0) {
        ...
        double E_over_T = pdotu / Tdec;
        delta_f_bulk =
            (-1. * (1. - sign * f0) * bulkPi / sqrt(E_over_T)
             * (-bulkvisCoefficients[0] + bulkvisCoefficients[1] * E_over_T));
    } else if (deltaf_kind_ == 4) {
        double E_over_T = pdotu / Tdec;
        delta_f_bulk =
        ...
        delta_f_bulk =
             ...
             * (mass * mass * bulkvisCoefficients[2]
                + pdotu
                      * (baryon * bulkvisCoefficients[3]
                         + strange * bulkvisCoefficients[4]
                         + charge * bulkvisCoefficients[5])
                + pdotu * pdotu
                      * (bulkvisCoefficients[1] - bulkvisCoefficients[2])));
    } else if (deltaf_kind_ == 21) {
...
int FSSW::sample_momemtum_from_a_fluid_cell(
    const double mass, const int sign, const int baryon, const int strange,
    const int charge, const FO_surf_LRF *surf,
    const std::vector<double> visCoefficients, const double deltaf_qmu_coeff,
    double &pT, double &phi, double &y_minus_eta_s) {
    const double Tdec = surf->Tdec;
    const double mu = std::min(
        mass,
        static_cast<double>(
            baryon * surf->muB + strange * surf->muS + charge * surf->muQ));
    ...
    const double dsigam_fac =
        (std::abs(surf->da_mu_LRF[0])
         + sqrt(
             surf->da_mu_LRF[1] * surf->da_mu_LRF[1]
    ...
    while (tries < maximum_impatience) {
        ...
        double p_mag =
            momentum_sampler_ptr_->Sample_a_momentum(mass, Tdec, mu, sign);
        ...
        double pdsigma =
            (p0 * surf->da_mu_LRF[0] + px * surf->da_mu_LRF[1]
        ...
        if (INCLUDE_BULK_DELTAF) {
            ...
            delta_f_bulk = get_deltaf_bulk(
                mass, p0, bulkPi, Tdec, sign, baryon, strange, charge, f0,
                visCoefficients);
        }
        ...
        if (INCLUDE_DIFFUSION_DELTAF) {
            double qmufactor =
                (-px * surf->qmuLRF_x - py * surf->qmuLRF_y
                 - pz * surf->qmuLRF_z);
            delta_f_qmu =
                ((1. - sign * f0) * (prefactor_qmu - baryon / p0) * qmufactor
                 / deltaf_qmu_coeff);
        }

        double deltaf_total = delta_f_shear + delta_f_bulk + delta_f_qmu;
        double deltaf_regulated =
            deltaf_total
            * (std::min(
                1., deltaf_max_ratio_ / (std::abs(deltaf_total) + 1e-16)));
        ...
        if (ran_gen_ptr->rand_uniform() < accept_prob) {
            ...
            VecD4 umu = {
                surf->u_tz[0], surf->u_tz[1], surf->u_tz[2], surf->u_tz[3]};
            VecD4 pLab = {0., 0., 0., 0.};
...
void FSSW::add_one_sampled_particle(
    ...
    const int particle_monval, const double mass, const double pT,
    const double phi, const double y_minus_eta_s, const double eta_s) {
...
void FSSW::boost_vector_back_to_lab_frame(
    ...
    double p_dot_u = 0.;
    for (int i = 1; i < 4; i++) p_dot_u += p_LRF[i] * umu[i];
...
double FSSW::bilinearInterp(
    std::vector<std::vector<double>> &mat, int idx_e1, int idx_e2, int idx_nB1,
    int idx_nB2, double x_fraction, double y_fraction) {
