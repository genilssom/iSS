...
class FSSW {
  private:
    ...
    std::vector<double> deltaf_coeff_newRTA_bulk_cs2_;
  ...
  public:
    ...
    int sample_momemtum_from_a_fluid_cell(
        ...
        const double deltaf_qmu_coeff, double &pT, double &phi,
        double &y_minus_eta_s);
    ...
    void boost_vector_back_to_lab_frame(
        iSS_data::VecD4 &p_LRF, iSS_data::VecD4 &p_lab,
        iSS_data::VecD4 &umu) const;
    double bilinearInterp(
        std::vector<std::vector<double>> &mat, int idx_e1, int idx_e2,
        int idx_nB1, int idx_nB2, double x_fraction, double y_fraction);
};
